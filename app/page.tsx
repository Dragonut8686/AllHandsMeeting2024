...
const Snowflake = dynamic(() => import("./components/Snowflake"), {
  ssr: false,
});
...
export default function Home() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (newData.length > 0 && timestamp > lastUpdateTimestamp.current) {
      setUsers((prevUsers) => {
        ...
        // If there are new users, insert them at the current position
        ...
        return newData; // If no new users, replace with the fetched data
      });
  ...
  useEffect(() => {
    setMounted(true);
    fetchData();
    const fetchIntervalId = setInterval(fetchData, 6000); // Fetch every 6 seconds
  ...
  useEffect(() => {
    if (mounted && users.length > 0) {
      ...
      }, 10000); // Change display every 10 seconds
        ...
        <main className="flex flex-col sm:flex-row flex-grow p-4 space-y-4 sm:space-y-0 sm:space-x-4 overflow-hidden">
          <div className="w-full sm:w-1/2 h-1/2 sm:h-full">
            <div className="bg-[#1F1F1F] rounded-3xl p-6 h-full flex flex-col">
              <div className="border-b border-[#F0F0F0]/20 pb-4 mb-4 flex-shrink-0">
                <h2 className="text-xl font-semibold text-[#F0F0F0]/70">
                  Чем горжусь в этом году
                </h2>
              </div>
              <div className="flex-grow overflow-hidden">
                <AnimatedText
                  key={`left-${currentUserIndex}`}
                  text={currentUser.leftPanelText}
          ...
          <div className="w-full sm:w-1/2 h-1/2 sm:h-full">
            <div className="bg-[#1F1F1F] rounded-3xl p-6 h-full flex flex-col">
              ...
              <div className="flex-grow overflow-hidden">
                <AnimatedText
                  key={`right-${currentUserIndex}`}
                  text={currentUser.rightPanelText}
                  className="text-2xl font-bold"
                />
              </div>
            </div>
